...
class TestExample {
    ...
    class InClassExample {
        ...
        //public static final PostgreSQLContainer<?> postgres = new PostgreSQLContainer<>(DockerImageName.parse("postgres").withTag("16-alpine"))
          //      .withReuse(true); // Ensure that testcontainers.reuse.enable=true is set in ~/.testcontainers.properties

        @BeforeAll
        public static void beforeAll() {
            postgreSQLContainer.start(); //When is started once, when another class executes his start, it will use this one
        }
